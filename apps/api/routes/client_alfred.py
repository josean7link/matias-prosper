...
async def _handle_customer_event(*, customer_id: str, evt_type: str,
    ...
    if is_kyb_approved or is_kyb_rejected:
        ...
        if is_kyb_approved:
            ...
            # Provision Andes ramp account (Phase 14) idempotently.
            # Tolerant to missing KYC docs — see ramp_routes.ensure_org_ramp_account.
            try:
                ...
                if org_doc:
                    await ensure_org_ramp_account(
                        ...
                        end_customer_id=org_doc["org_id"],
                        display_name=(org_doc.get("commercial_name")
                                       or org_doc.get("legal_name")),
                        holder_name=(org_doc.get("legal_name")
                                      or org_doc.get("commercial_name")),
                        holder_tax_id=org_doc.get("tax_id"))
            except Exception as e:  # noqa: BLE001
                logger.exception("ensure_org_ramp_account after KYB failed: %s", e)
        return
...
async def transactions_history(
    ...
    """Unified movements feed for the client portal (Feb 2026 rewrite).

    Sources merged into one chronological feed:
      * `ramp_movements`  → ARSa deposits (CVU credits) & withdrawals
                            & on-chain transfers (e.g. Andes outbound to
                            Prosper Treasury when invested).
      * `positions`       → one row per investment lifecycle stage:
                            invest (creation), redeem (when status flips).
      * `onramp_orders`   → Alfred USDC fiat onramp (only confirmed/
                            pending — mock-only orders are excluded).
      * `offramp_orders`  → Alfred offramp (same filter).
    
    DELIBERATELY EXCLUDED:
      * `transactions` collection rows — those are legacy "intent" stubs
        from the deprecated ARSa→USDC bridge era (failed/cancelled
        subscribe entries leaking English errors in memos). They never
        moved real money and have no place in a customer-facing log.
        They're retained in the DB for internal audit and surface under
        `/admin/legacy-intents` only.

    Unified row shape:
        tx_id, type, status, amount, asset, currency_label, created_at,
        memo (humanized ES), tx_hash?, destination_cvu?, position_id?
    """
    org_id = user.org_id
    rows: list[dict] = []

    # 1) ARSa CVU + on-chain transfers (ramp_movements)
    async for m in col("ramp_movements").find(
            {"org_id": org_id}, {"_id": 0}).sort("created_at", -1).limit(500):
        kind   = (m.get("kind") or "").lower()
        m_amt  = float(m.get("amount") or 0)
        m_stat = _normalize_movement_status(m.get("status"))
        m_at   = m.get("occurred_at") or m.get("created_at") or _iso_now()
        if kind == "deposit":
            row_type, row_memo = "arsa_deposit", _memo_for_deposit(m)
        elif kind in ("withdraw", "withdrawal"):
            row_type, row_memo = "arsa_withdraw", _memo_for_withdraw(m)
        elif kind == "transfer":
            # User transferred ARSa out of their Andes wallet (typically to
            # the Prosper Treasury — that's the "invertir ahora" flow). We
            # show this as an "invest" event so the user sees it in the
            # same place as their position; the position itself is the
            # other half (created by staking_sync poller).
            row_type, row_memo = "invest", _memo_for_transfer(m)
        else:
            continue
        rows.append({
            "tx_id":           f"mv_{m.get('id') or m.get('external_id') or 'unknown'}",
            "type":            row_type,
            "status":          m_stat,
            "amount":          m_amt,
            "asset":           (m.get("asset") or "arsa").lower(),
            "created_at":      m_at,
            "memo":            row_memo,
            "tx_hash":         m.get("tx_hash"),
            "destination_cvu": m.get("destination_cvu"),
            "position_id":     None,
            "source":          "ramp_movements",
        })

    # 2) Positions — one event per lifecycle. Active and pending positions
    # surface as "Inversión"; redeemed positions emit BOTH the inversión
    # (completed) and a "Rescate" row so the activity log tells the full
    # story.
    async for p in col("positions").find(
        ...
        principal     = float(p.get("principal_native")
                                or p.get("principal_usd") or 0)
        p_asset       = (p.get("asset") or "usdc").lower()
        p_id          = p.get("position_id")
        p_modality    = (p.get("modality") or "end").lower()
        # Original "invest" event
        invest_status = ("confirmed"        if p_status in ("active", "matured")
                          else "pending_onchain" if p_status == "pending_onchain"
                          else "cancelled"  if p_status == "expired_pending_onchain"
                          else "confirmed"  if p_status == "redeemed"
                          else "pending")
        rows.append({
            "tx_id":           f"pos_invest_{p_id}",
            "type":            "invest",
            "status":          invest_status,
            "amount":          principal,
            "asset":           p_asset,
            "created_at":      p.get("start") or p.get("created_at") or _iso_now(),
            "memo":            _memo_for_invest(p_modality, p_asset, p_status),
            "tx_hash":         p.get("hash") or p.get("andes_tx_hash"),
            "destination_cvu": None,
            "position_id":     p_id,
            "source":          "positions",
        })
        # Redeem row when the position closed
        if p_status == "redeemed":
            rows.append({
                "tx_id":           f"pos_redeem_{p_id}",
                "type":            "redeem",
                "status":          "confirmed",
                "amount":          principal + float(p.get("claimed_interest") or 0),
                "asset":           p_asset,
                "created_at":      p.get("redeemed_at")
                                       or p.get("updated_at")
                                       or _iso_now(),
                "memo":            f"Capital + intereses devueltos a tu cuenta {p_asset.upper()}.",
                "tx_hash":         p.get("hash"),
                "destination_cvu": None,
                "position_id":     p_id,
                "source":          "positions",
    ...
    # 3) Alfred onramp orders (USDC fiat onramp) — only live partner flows
    async for o in col(ONRAMP_ORDERS).find(
            {"org_id": org_id, "is_deleted": {"$ne": True}},
            {"_id": 0}).sort("created_at", -1).limit(200):
        o_status = _normalize_alfred_status(o.get("status"))
        if o_status is None:   # mock-only / dead order → skip
            continue
        rows.append({
            "tx_id":           f"ramp_{o.get('order_id') or o.get('alfred_id')}",
            "type":            "usdc_deposit",
            "status":          o_status,
            "amount":          float(o.get("usdc_received") or o.get("amount") or 0),
            "asset":           "usdc",
            "created_at":      o.get("created_at") or _iso_now(),
            "memo":            _memo_for_usdc_deposit(o, o_status),
            "tx_hash":         o.get("tx_hash"),
            "destination_cvu": None,
            "position_id":     None,
            "source":          "onramp_orders",
        })

    # 4) Alfred offramp orders (USDC → fiat) — only live partner flows
    async for o in col(OFFRAMP_ORDERS).find(
            {"org_id": org_id, "is_deleted": {"$ne": True}},
            {"_id": 0}).sort("created_at", -1).limit(200):
        o_status = _normalize_alfred_status(o.get("status"))
        if o_status is None:
            continue
        rows.append({
            "tx_id":           f"out_{o.get('order_id') or o.get('alfred_id')}",
            "type":            "arsa_withdraw" if (o.get("payout_asset") or "").lower() == "ars"
                                else "usdc_withdraw",
            "status":          o_status,
            "amount":          float(o.get("amount") or 0),
            "asset":           (o.get("amount_asset") or "usdc").lower(),
            "created_at":      o.get("created_at") or _iso_now(),
            "memo":            _memo_for_usdc_withdraw(o, o_status),
            "tx_hash":         o.get("tx_hash"),
            "destination_cvu": o.get("destination_cvu") or o.get("payout_cvu"),
            "position_id":     None,
            "source":          "offramp_orders",
        })
    ...
    # 6) Sort by created_at desc + truncate
    rows.sort(key=lambda r: r.get("created_at") or "", reverse=True)
    return {"items": rows[:limit], "total": len(rows)}


# ---------------------------------------------------------------------------
# Helpers — keep movement / order semantics in one place so the client
# never sees raw provider statuses or internal error messages.
# ---------------------------------------------------------------------------
def _iso_now() -> str:
    return datetime.now(timezone.utc).isoformat()


def _normalize_movement_status(raw: Any) -> str:
    """Map Andes ramp_movements.status (Success/Pending/Failed/...) to
    our unified Spanish-portal vocabulary."""
    s = (str(raw or "")).lower()
    if s in ("success", "completed", "confirmed", "settled"):
        return "confirmed"
...
def _normalize_alfred_status(raw: Any) -> Optional[str]:
    """Map Alfred onramp/offramp order status to portal vocabulary, or
    return None for mock/throwaway entries (NOT shown to the client)."""
    s = (str(raw or "")).lower()
    if s in ("mock", "draft", "init", "initialised", "initialized"):
        return None  # never made it past the mock layer → hide
    if s in ("settled", "completed", "confirmed", "trade_completed",
              "on_chain_received"):
        return "confirmed"
    if s in ("failed", "error", "on_chain_failed", "rejected"):
        return "failed"
...
def _memo_for_deposit(m: dict) -> str:
    chain = (m.get("chain") or "").upper()
    if chain == "STELLAR":
        return "Depósito recibido en tu cuenta Prosper."
    return "Depósito acreditado por CVU bancario."


def _memo_for_withdraw(m: dict) -> str:
    cvu = m.get("destination_cvu") or m.get("destination_alias")
    return (f"Retiro a {cvu}." if cvu
              else "Retiro a tu cuenta bancaria.")


def _memo_for_transfer(m: dict) -> str:
    to = (m.get("to_address") or "")
    return ("Inversión iniciada — transferencia on-chain en proceso."
              if to else "Transferencia on-chain.")


def _memo_for_invest(modality: str, asset: str, p_status: str) -> str:
    mod_label = "mensual" if modality == "month" else "al vencimiento"
    unit      = asset.upper()
    if p_status == "pending_onchain":
        return f"Inversión {unit} ({mod_label}) — detectando depósito on-chain."
    if p_status == "expired_pending_onchain":
        return ("La inversión no se completó. Tu dinero no se movió. "
                  "Volvé a intentar desde Invertir.")
    if p_status == "matured":
        return f"Inversión {unit} ({mod_label}) — vencida, lista para rescatar."
    if p_status == "redeemed":
        return f"Inversión {unit} ({mod_label}) — completada."
    return f"Inversión {unit} ({mod_label}) — activa."


def _memo_for_usdc_deposit(o: dict, status: str) -> str:
    if status == "failed":
        return ("La carga no se completó. Tu dinero no se movió. "
                  "Si te debitaron, te lo devolvemos en 24-48hs.")
    if status == "cancelled":
        return "Carga cancelada."
    if status == "pending":
        return "Carga en curso — esperando acreditación."
    return "Carga acreditada en tu cuenta USDC."


def _memo_for_usdc_withdraw(o: dict, status: str) -> str:
    if status == "failed":
        return ("El retiro no se completó. Tu dinero no se movió. "
                  "Si te debitaron, te lo devolvemos en 24-48hs.")
    if status == "cancelled":
        return "Retiro cancelado."
    if status == "pending":
        return "Retiro en proceso."
    cvu = o.get("destination_cvu") or o.get("payout_cvu")
    return f"Retiro acreditado en {cvu}." if cvu else "Retiro acreditado."
