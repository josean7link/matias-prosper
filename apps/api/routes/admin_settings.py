...
CATALOG: List[ProviderSpec] = [
    ...
    ProviderSpec(
        ...
        description="Emisión y redemption del token USDCP sobre Stellar. NAV publication + treasury ops.",
        supports_mode=True, supports_test=True, badge_color="primary",
        docs_default=[{"title": "Stellar SDK", "url": "https://developers.stellar.org/docs"},
                      ...
                      {"title": "Prosper API (dev)",
                       "url": "https://apidev.protocol-prosper.io/api/docs"}],
        fields=[
            FieldSpec(key="api_user", label="API username",
                     placeholder="matias@alfredpay.io", env_var="PROSPER_API_USER"),
            FieldSpec(key="api_pass", label="API password", secret=True,
                     env_var="PROSPER_API_PASS"),
            FieldSpec(key="api_base", label="API base URL",
                     placeholder="https://apidev.protocol-prosper.io",
                     env_var="PROSPER_API_BASE"),
            FieldSpec(key="asset_code", label="Asset code",
                     placeholder="USDCP", env_var="PROSPER_ASSET_CODE"),
            FieldSpec(key="issuer_pubkey", label="Issuer public key",
                     placeholder="G… (Stellar account)",
                     env_var="PROSPER_ISSUER_PUBLIC_KEY"),
            FieldSpec(key="distribution_pubkey", label="Distribution public key / contract",
                     placeholder="C… (Soroban contract) o G… (account)",
                     env_var="PROSPER_DISTRIBUTION_PUBLIC_KEY"),
            FieldSpec(key="distribution_secret", label="Distribution secret (opcional)",
                     secret=True, required=False,
                     env_var="STELLAR_DISTRIBUTION_SECRET"),
        ]),
    ProviderSpec(
        provider="alfredpay", name="Alfred · Onramp/Offramp/KYC",
        category="onramp",
        description="Rampas fiat↔crypto (ARS/BRL/MXN/CLP) + KYB/KYC hosteado.",
        supports_mode=True, supports_test=True, badge_color="success",
        docs_default=[{"title": "Alfred Penny API",
                       "url": "https://alfredpay.readme.io/reference"}],
        fields=[
            FieldSpec(key="api_key", label="API key", secret=True,
                     env_var="ALFRED_API_KEY"),
            FieldSpec(key="api_secret", label="API secret", secret=True,
                     env_var="ALFRED_API_SECRET"),
            FieldSpec(key="business_id", label="Business ID",
                     placeholder="187", env_var="ALFRED_BUSINESS_ID"),
            FieldSpec(key="base_sandbox", label="Base URL sandbox",
                     env_var="ALFRED_API_BASE_SANDBOX",
                     placeholder="https://penny-api-restricted-dev.alfredpay.io/api/v1/third-party-service/penny"),
            FieldSpec(key="base_production", label="Base URL production",
                     env_var="ALFRED_API_BASE_PRODUCTION",
                     placeholder="https://penny-api-restricted.alfredpay.io/api/v1/third-party-service/penny"),
            FieldSpec(key="webhook_secret", label="Webhook HMAC secret",
                     secret=True, env_var="ALFRED_WEBHOOK_SECRET"),
            FieldSpec(key="kyc_mode", label="KYC mode (mock|sandbox|production)",
                     required=False, placeholder="sandbox", env_var="ALFRED_KYC_MODE"),
        ]),
    ProviderSpec(
...
class IntegrationPatch(BaseModel):
    ...
    # Accepted modes:
    #   sandbox / live / production → real provider call
    #   simulated                   → force local simulator (Phase 22+ KYC toggle)
    mode:   Optional[Literal["sandbox", "live", "production", "simulated"]] = None
