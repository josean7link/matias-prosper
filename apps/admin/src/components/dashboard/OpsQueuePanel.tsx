...
export function OpsQueuePanel({ data, loading, wsStatus }: { data: OpsQueue | undefined; loading?: boolean; wsStatus?: WsStatus }) {
  const t = useTranslations("admin.ops_queue");
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-surface">
        <div>
          ...
          >
            {wsStatus === "live" ? t("live_ws")
              : wsStatus === "connecting" ? t("connecting")
              : t("live_30s")}
          </div>
          <h3 className="font-display font-bold text-sm text-fg mt-0.5">{t("title")}</h3>
        </div>
      ...
      {data && (
        <div className="px-4 py-2 border-t border-border bg-surface text-[10px] font-mono text-fg-subtle uppercase tracking-wider">
          {t("last_refresh", { time: new Date(data.generated_at).toLocaleTimeString(undefined, { hour12: false }) })}
        </div>
      )}
