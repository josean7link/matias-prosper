...
export function ClientGateBanner() {
  ...
  const sanctions   = org.sanctions_status   || "pending";
  const travelRule  = org.travel_rule_status || "pending";

  if (org.kyb_status === "approved" && sanctions === "clear"
      && ["clear", "na"].includes(travelRule) && !paused) return null;
  ...
  // Sanctions/PEP gate has precedence over kyb_status when both block.
  // Andes can have approved you (kyb_status would normally flip) but the
  // sanctions queue might still be pending or flagged.
  if (sanctions === "flagged") {
    tone = "danger";
    icon = <XCircle size={16} />;
    title = "Cuenta bloqueada por compliance";
    msg = "Detectamos un match en listas de sanciones/PEP. Compliance va a contactarte por email para resolver.";
    cta = null;
  } else if (travelRule === "flagged") {
    tone = "danger";
    icon = <XCircle size={16} />;
    title = "Cuenta bloqueada por travel-rule";
    msg = "Compliance detectó una inconsistencia en los datos de travel-rule. Te vamos a contactar para resolver.";
    cta = null;
  } else if (sanctions === "pending" && org.kyb_status !== "approved") {
    // KYB still in progress + sanctions pending: keep KYB-style messaging.
    // We'll override below if KYB itself has more specific state.
  } else if (sanctions === "pending") {
    tone = "info";
    icon = <Clock size={16} />;
    title = "Verificación de antecedentes en curso";
    msg = "Andes ya validó tu identidad. Estamos completando el screening de sanciones / PEP — suele tardar menos de 1 día hábil.";
    cta = null;
  } else if (travelRule === "pending") {
    tone = "info";
    icon = <Clock size={16} />;
    title = "Validando datos travel-rule";
    msg = "Identidad y sanctions OK. Estamos cerrando el último check (travel-rule FATF/UIF) — suele tardar menos de 1 día hábil.";
    cta = null;
  }
  ...
  if (paused) {
    ...
    cta = null;
  } else if (sanctions !== "flagged" && travelRule !== "flagged"
              && org.kyb_status === "in_review") {
    tone = "info";
    icon = <Clock size={16} />;
    ...
    cta = null;
  } else if (sanctions !== "flagged" && org.kyb_status === "rejected") {
    tone = "danger";
    icon = <XCircle size={16} />;
    title = "KYB rechazado";
    ...
    cta = null;
  } else if (sanctions !== "flagged" && org.kyb_status === "needs_info") {
    tone = "warning";
