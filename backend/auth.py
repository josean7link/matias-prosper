...
def make_jwt(*, user_id: str, email: str, role: Role, org_id: Optional[str],
              ...
              org_type: Optional[str] = None) -> str:
    ...
    payload = {
        ...
        "role": role.value,
        "org_id": org_id,
        # P1·#6 (Feb 2026) — org_type is carried in the JWT so the Next.js
        # Edge middleware can gate partner-only routes (/client/api-keys,
        # /webhooks, /widget, /developers) WITHOUT a DB roundtrip. Legacy
        # sessions minted before this change won't have the claim; the
        # frontend falls back to "personal" (deny) in that case which is
        # the safer default.
        "org_type": org_type,
...
async def mint_session_token(
    *, user_id: str, email: str, role: Role, org_id: Optional[str],
    ...
    # Resolve org_type for the claim so the Edge middleware can gate
    # partner-only routes without a DB hop on every request.
    org_type: Optional[str] = None
    if org_id:
        org_doc = await col(ORGANIZATIONS).find_one(
            {"org_id": org_id}, {"_id": 0, "type": 1})
        if org_doc:
            org_type = org_doc.get("type")
    return make_jwt(user_id=user_id, email=email, role=role,
                    org_id=org_id, jti=jti, org_type=org_type)
