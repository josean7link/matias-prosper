...
from routes.ramp_routes import router as ramp_router
from routes.ramp_international import router as ramp_intl_router
from routes.ramp_webhook import router as ramp_webhook_router
from routes.subclients import router as subclients_router
from routes.phase20_bridge import router as phase20_bridge_router
from routes.phase20_admin import router as phase20_admin_router
from routes.sanctions import router as sanctions_router
from routes.travel_rule import router as travel_rule_router
from routes.phase22_admin_yield import router as phase22_admin_yield_router
...
from routes.admin_deposits import router as admin_deposits_router
from routes.client_notifications import router as client_notifications_router
from routes.client_portfolio import router as client_portfolio_router
...
async def startup():
    ...
    # Phase 13 — Ramp provider abstraction: seed the default `scope=global`
    # row (alfred/mock) so a fresh DB has a sensible config; no-op if a row
    # already exists.
    from ramp.registry import ensure_default_provider_config
    ...
    from services.deposit_credited_handler import on_event as _on_dep_credit
    subscribe("deposit.credited", _on_dep_credit)
...
async def _send_otp(email: str, code: str):
    ...
    body = f"""
      ...
      <p>Usá este código para entrar a Prosper. Vence en 10 minutos.</p>
      <p style="font-family:'IBM Plex Mono',monospace;font-size:34px;font-weight:700;
                letter-spacing:6px;background:#F2F6FF;border-radius:10px;
                padding:18px 24px;display:inline-block;color:#2563FF">{code}</p>
      <p style="font-size:12px;color:#6B7280">Si no pediste este código, ignorá este mail.</p>"""
    try:
        rec = await send_email(
            to=email, subject="Tu código de acceso · Prosper",
...
async def passwordless_token(request: Request, body: TokenIn, response: Response):
    ...
    # Resolve user. Auto-provisioning is restricted to @prosper.foundation
    # (internal staff). External emails MUST be pre-registered via
    # `/onboarding/apply` to avoid the historical bug where a new client
    # email would auto-attach to whichever org happened to share its
    # domain in its allowlist.
    ...
    if not user_doc:
        ...
        if domain != "prosper.foundation":
            raise HTTPException(
                403,
                "Este email no está registrado. Abrí tu cuenta en /apply.")
        # @prosper.foundation auto-onboarding defaults to plain admin — only
        # explicitly seeded users (see seed.seed_phase1) hold super_admin.
        user_doc = {
            "user_id": f"usr_{secrets.token_hex(6)}",
            "email": email, "role": Role.admin.value,
            "org_id": None,
            "status": "active",
    ...
    # querystring that came from /apply / external links).
    return {"accessToken": access, "portal": _portal_for_role(role),
              "role": role.value}
...
# Sets the prosper_session cookie and 302s to a destination decided BY ROLE
# (internal → /admin, client → /client). The `?next=` param is honoured only
# when compatible with the user's role family; otherwise it's discarded so
# a client cannot follow an admin link by accident.
# ---------------------------------------------------------------------------
def _portal_for_role(role: Role) -> str:
    return "/admin" if is_internal(role) else "/client"


def _safe_next(req_next: str, role: Role) -> str:
    """Honour `next` only if it matches the role's portal family."""
    default = _portal_for_role(role)
    if not req_next.startswith("/"):
        return default
    # Strip admin destinations for client roles and vice versa
    if req_next.startswith("/admin") and not is_internal(role):
        return "/client"
    if req_next.startswith("/client") and is_internal(role):
        # Internal user landing on /client by accident → send to /admin
        return "/admin"
    return req_next
...
async def dev_login(email: str, request: Request, next: str = ""):
    ...
    if not user_doc:
        domain = email.rsplit("@", 1)[-1]
        # Same hardening as `/auth/passwordless-token`: only internal
        # @prosper.foundation gets auto-provisioned. External emails must
        # exist in USERS already (created by `/onboarding/apply`).
        if domain != "prosper.foundation":
            raise HTTPException(
                403,
                "Este email no está registrado. Abrí tu cuenta en /apply.")
...
def _feature_flags(user_doc: dict, org_doc: Optional[dict]) -> dict:
    sanctions   = (org_doc or {}).get("sanctions_status") or "pending"
    travel_rule = (org_doc or {}).get("travel_rule_status") or "pending"
    return {
        ...
        "sanctions_locked":     sanctions != "clear",
        "travel_rule_status":   travel_rule,
        "travel_rule_locked":   travel_rule not in ("clear", "na"),
...
api.include_router(client_kyc_router, prefix="/v1")
...
api.include_router(ramp_router, prefix="/v1")
api.include_router(ramp_intl_router, prefix="/v1")
api.include_router(ramp_webhook_router, prefix="/v1")
api.include_router(subclients_router, prefix="/v1")
api.include_router(phase20_bridge_router, prefix="/v1")
api.include_router(phase20_admin_router, prefix="/v1")
api.include_router(phase22_admin_yield_router, prefix="/v1")
api.include_router(ramp_public_webhook_router, prefix="/v1")
api.include_router(admin_ramp_router, prefix="/v1")
api.include_router(sanctions_router, prefix="/v1")
api.include_router(travel_rule_router, prefix="/v1")
api.include_router(admin_deposits_router, prefix="/v1")
