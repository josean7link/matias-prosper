...
Covers:
  * GET /client/balances (mock mode shape)
  * Product max_amount + min_amount per product
  * Auto-buy is SKIPPED when onramp.usdc_received < liquid_v1.min_amount (40k ARS)
  * GET /client/positions/{id} returns position + events (subscribe present)
  * POST /client/positions on non-matured term position → cannot redeem (400)
  * KYB pending org (finpact) blocked from manual buy (403)
  * Accrual idempotency (calling run_accrual_once twice the same UTC day does
    not double-accrue an active position).
"""
