...
logger = logging.getLogger("prosper.admin.ops")
...
async def seed_demo_client(body: SeedDemoClientIn,
    ...
    # Phase 14 — auto-provision Andes ramp account for demos that are
    # KYB-approved up-front. Tolerant to missing KYC docs (the demo flow
    # doesn't upload any → expect onboarding_status=kyc_pending_andes when
    # the provider is real; mock provider returns approved + a CVU).
    if body.auto_approve:
        try:
            from routes.ramp_routes import ensure_org_ramp_account
            await ensure_org_ramp_account(
                org_id=org_id, end_customer_id=org_id,
                display_name=commercial, holder_name=legal)
        except Exception as e:  # noqa: BLE001
            logger.exception("seed_demo_client → ramp account create: %s", e)
