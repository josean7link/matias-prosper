...
export default function Dashboard() {
  ...
  if (!data) return <div className="text-[var(--fg-muted)] font-mono text-sm">Loading control room…</div>;
  const k = data.kpis || {};
    ...
    <div data-testid="dashboard-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-6">
        <div className="lg:col-span-2 prosper-card p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)]">NAV per token · {data.primary_fund_code}</div>
            ...
            <div className="text-[10px] font-mono uppercase text-[var(--fg-muted)]">14 days</div>
          </div>
          <div className="h-[220px]">
            <ResponsiveContainer>
              <AreaChart data={data.nav_series || []}>
                ...
                <Tooltip contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--fg)", borderRadius: "8px", fontSize: 12 }}
        ...
        <div className="prosper-card p-5">
          <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] mb-4">Ops Queue</div>
      ...
      <div className="prosper-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)]">Transaction Volume · 14d</div>
        </div>
        <div className="h-[220px]">
          <ResponsiveContainer>
            <BarChart data={data.volume_series || []}>
              <CartesianGrid stroke="var(--border)" vertical={false} />
              <XAxis dataKey="date" stroke="var(--fg-muted)" fontSize={10} tickFormatter={(v) => v.slice(5)} />
              <YAxis stroke="var(--fg-muted)" fontSize={10} tickFormatter={(v) => fmtCompact(v)} />
              <Tooltip contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--fg)", borderRadius: "8px", fontSize: 12 }}
