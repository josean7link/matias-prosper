...
const Section = ({ title, accounts }) => {
  ...
  return (
    <section className="mb-8">
      <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] mb-2 font-mono">{title}</div>
              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] font-mono">{a.custody}</span>
                  </div>
                  <div className="font-display font-bold text-lg">{a.label}</div>
                </div>
                <StatusBadge value={a.environment} />
              </div>
              <div className="grid grid-cols-2 tight-grid border border-[var(--border)] mb-3">
                <div className="p-3 bg-[var(--surface)]">
                  <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)]">{a.asset_code || "Asset"} balance</div>
                  <div className="font-mono text-lg mt-1">{fmtNum(a.balance_asset, 2)}</div>
                </div>
                <div className="p-3 bg-[var(--surface)]">
                  <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)]">XLM</div>
