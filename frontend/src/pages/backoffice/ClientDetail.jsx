...
export default function ClientDetail() {
  ...
  if (!org) return <div className="text-[var(--fg-muted)] font-mono text-sm">Loading client…</div>;

  return (
    <div data-testid="client-detail-page">
      <button onClick={() => nav(-1)} className="flex items-center gap-2 text-xs text-[var(--fg-muted)] hover:text-[var(--fg)] mb-4 font-mono uppercase tracking-wider" data-testid="back-button">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-6">
        <section className="prosper-card p-5">
          <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] mb-3">Contact</div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-[var(--fg-muted)]">Email</span><span>{org.contact_email || "—"}</span></div>
            <div className="flex justify-between"><span className="text-[var(--fg-muted)]">Country</span><span>{org.country || "—"}</span></div>
            <div className="flex justify-between"><span className="text-[var(--fg-muted)]">Environment</span><EnvPill env={org.environment} /></div>
            <div className="flex justify-between"><span className="text-[var(--fg-muted)]">Created</span><span className="font-mono text-xs">{fmtDate(org.created_at)}</span></div>
          </div>
        </section>

        <section className="prosper-card p-5">
          <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] mb-3">API Keys</div>
          {keys.length === 0 ? <div className="text-sm text-[var(--fg-subtle)]">No keys issued</div> :
            <div className="space-y-2">
              {keys.slice(0, 5).map((k) => (
                <div key={k.key_id} className="flex items-center justify-between py-2 border-b border-[var(--border)] last:border-0">
                  <div>
                    <div className="text-sm text-[var(--fg)]">{k.label}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-3">
        <section className="prosper-card p-5">
          <div className="text-[10px] uppercase tracking-wider text-[var(--fg-muted)] mb-3">Recent Positions</div>
            ...
            <tbody>
              {positions.slice(0, 6).map((p) => (
                <tr key={p.position_id}>
                  ...
                  <td className="py-2 text-right font-mono text-[var(--success)]">{fmtMoney(p.accrued_interest)}</td>
            ...
            <tbody>
              {txs.slice(0, 6).map((t) => (
                <tr key={t.tx_id}>
                  ...
                  <td className="py-2 text-xs font-mono text-[var(--fg-muted)]">{fmtDate(t.created_at, true)}</td>
                </tr>
              ))}
            </tbody>
          </table>
