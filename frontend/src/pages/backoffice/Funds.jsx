          ...
          {items.map((f) => (
            <div key={f.fund_id} className="prosper-card p-6" data-testid={`fund-card-${f.fund_id}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="font-mono text-xs text-[#888] mb-1">{f.code} · {f.underlying}</div>
              ...
              <div className="grid grid-cols-3 tight-grid border border-[#1a1a1a] mb-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <Row label="Home Domain" value={<span className="font-mono text-[#ccc]">{f.home_domain}</span>} />
                <Row label="Regulator" value={<span className="text-[#ccc]">{f.regulator}</span>} />
                <Row label="Rating" value={<span className="text-[#ccc]">{f.rating}</span>} />
              </div>
              <div className="mt-4 pt-4 border-t border-[#1a1a1a]">
                <div className="text-[10px] uppercase tracking-wider text-[#888] mb-2">Products</div>
                <div className="flex flex-wrap gap-2">
                  {(f.products || []).map((p) => (
                    <span key={p.product_id} className="text-xs px-2 py-1 rounded-sm border border-[#222] font-mono">
