        ...
        <div className="flex items-center gap-2">
          ...
          {counts && (
            <span className="text-[10px] font-mono text-fg-subtle">
              · <span className="text-success">{counts.ok}</span> {t("ok")}
              · <span className="text-warning">{counts.degraded}</span> {t("degraded")}
              · <span className="text-danger">{counts.missing}</span> {t("missing")}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
            ...
            <Link key={it.provider}
              ...
              {it.ran_now && (
                <span className="text-[9px] font-mono uppercase tracking-wider text-primary"
                      title="Re-tested on this request">{t("live")}</span>
              )}
            </Link>
          );
        })}
