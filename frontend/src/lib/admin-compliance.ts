...
export interface KycCase {
  ...
  // Two-gate state (only populated for andes-direct cases, prefix `app_…`)
  andes_kyc_status?: "pending" | "approved" | "rejected";
  ...
  andes_user_id?: string | null;
  sanctions?: {
    screening_id: string; status: "pending" | "clear" | "flagged";
    provider: string; resolution?: string;
    decided_by?: string | null; decided_at?: string | null; reason?: string | null;
  } | null;
  travel_rule?: {
    screening_id: string; status: "pending" | "clear" | "na" | "flagged";
    provider: string; resolution?: string;
    decided_by?: string | null; decided_at?: string | null; reason?: string | null;
  } | null;
}
...
export async function decideKyc(caseId: string,
    body: { action: "approve" | "reject" | "request_info"; reason: string;
             override_sanctions?: boolean; override_travel_rule?: boolean }) {
  return api<{ ok: boolean; case_id: string; status: string;
                sanctions_override_applied?: boolean;
                travel_rule_override_applied?: boolean }>(
    `/v1/admin/compliance/kyc/${caseId}/decision`, {
